...
#Plotly visulaizaton
def Plotly_Visualization(updated):
     dashboard(updated)  
def dashboard(updated):
    fig=make_subplots(
        rows=2,
    ...
    bar=px.bar(
            updated.groupby("Department")["Name"].count().reset_index(name="Employee count"),
            x="Department",
            y="Employee count",
            text="Employee count",
            color="Department",
    ...
    highest = updated["Salary"].idxmax()

    fig.add_annotation(
    x=updated.loc[highest, "Experience"],
    y=updated.loc[highest, "Salary"],
    text="Highest Salary",
    showarrow=True,
    arrowhead=2,
    row=2,
